...
Problem faced: Person had to represent the common information shared by customers and sellers while remaining abstract, and the specific attributes of Customer and Seller had to be kept in their corresponding subclasses.

Prompt used: ¿Qué cambios necesitan las clases Person, Customer y Seller para cumplir con el módulo de personas?
...
Problem faced: The repository needed to preserve the different types of people in the same people.txt file and reconstruct the correct object type when the application starts again.

Prompt used: ¿Cómo podemos guardar y cargar clientes y vendedores en PersonRepository utilizando un solo archivo?
...
Problem faced: The initial PersonService used generic methods for registering and listing Person objects, but the people module requires operations specifically related to customers and sellers.

Prompt used: ¿Cómo quedarían registerCustomer(), listCustomers() y listSellers() en PersonService y qué cambia en la clase?
...
Problem faced: It was necessary to confirm that the three sellers stored in people.txt were being loaded correctly without adding new records during the test.

Prompt used: ¿Cómo podemos comprobar que PersonRepository.load() está cargando correctamente los vendedores del archivo?
...
Problem faced: The original registerCustomer() method sent the customer directly to the repository, which could allow two customers with the same ID to be stored.
