...
Entry 6
Date: 2026-09-06, 3:16 PM
Tool used: Claude (web chat)

Reason for use: Decide where to place input validation logic (ID, name, phone, price, quantity, email, and the alphanumeric fields specific to VideoGame and Console) and get help implementing it in ConsoleUI once the rules were defined.

Problem faced: I wanted to validate raw console input (e.g., ID must contain digits only, phone must be exactly 10 digits, price must be a positive decimal) but did not know whether this logic should live in the UI layer, inside the model classes' setters, or in a separate validator class, nor the exact Java syntax (regular expressions, a reusable prompt-and-retry helper) to implement it cleanly without duplicating a while loop for every single field.

Prompt used: "¿cómo puedo yo... darte para que tú me ayudes a hacer las validaciones? Como, por ejemplo, el ID de una persona... que solamente debería ser exclusivamente número, o el nombre... exclusivamente caracteres..." followed by "Dame la opcion A" after the three placement options (UI, model setters, separate Validator class) were explained to me.

Solution obtained and decision taken: I chose Option A (validation lives directly in ConsoleUI, re-prompting the user until the input is valid) over validating inside the model setters or extracting a separate Validator class, to avoid touching my teammates' domain classes and to keep the console experience friendly. I then defined the exact rule for each field myself through a series of questions: ID and Employee Code (digits only), Name (letters and spaces, at least one letter), Title/Platform/Genre/Age Rating/Brand/Model/Generation (letters, numbers and spaces, at least one character — adjusted after initially restricting Title to letters only, since product titles like "FIFA 24" need to allow numbers), Phone (exactly 10 digits), Price (decimal, strictly greater than 0), Quantity (whole number, strictly greater than 0), and Email (must contain "@"). Based on these rules, I implemented a generic `readValidated(prompt, validator, errorMessage)` helper method and six small validation predicates using regular expressions inside ConsoleUI, plus two dedicated methods (`readValidPrice`, `readValidQuantity`) that parse and range-check numeric input. All business rules and field constraints were decided by me; the AI's contribution was explaining the three architectural placement options and their trade-offs, and providing the corresponding Java syntax (regex patterns, functional interface usage with Predicate) once I had made the design decisions.